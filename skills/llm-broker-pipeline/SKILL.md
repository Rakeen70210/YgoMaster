...
| Action kinds | `move_phase`, `command`, `dialog_result`, `list_index` (strict single-select only: `is_multi_mode==0`, min==max==1), and validated cancellable `CheckTiming`/`CheckChain` decline `cancel` (`CancelCommand2(false)`). Menu 4 decline taxonomy is `check_timing_decline`; menu 5 is `check_chain_decline`. When either response window is cancellable and the only extractable choice is empty-window decline, it is **mechanical** and auto-committed with **no model call**. Unchanged semantic optional-response windows reuse decline then temporary CPU (3C). Provider-error recovery prefers optional decline over list-order activation (3D). Generic skip/pass and multi-select remain unsupported → CPU/temporary-CPU fallback. |
